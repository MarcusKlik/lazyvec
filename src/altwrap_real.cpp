...
{
  return R_new_altrep(altwrap_real_class, data, NILSXP);
}
...
SEXP altwrap_real_UnserializeEX_method(SEXP altwrap_class, SEXP state, SEXP attr, int objf, int levs)
{
  Rcpp::Environment pkgs = Rcpp::Environment::namespace_env("lazyvec");
  ...
  SEXP altwrap_integer = PROTECT(altrep_real_wrapper(altrep_data1));
...
double altwrap_real_Elt_method(SEXP sx, R_xlen_t i)
{
  ...
  SET_VECTOR_ELT(arguments, 1, Rf_ScalarReal(element));
