...
> This repository is home to the `lazyvec` package. `lazyvec` depends heavily on the ALTREP framework that
will be rolled out with `R` 3.6. Therefore, at the moment, it can only be compiled from source when you are
running the latest `dev` version of `R`. After the `R` 3.6 release, `lazyvec` will be submitted to CRAN and more readily available to the user.
